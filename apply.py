...
def goLayers():
  ...
  fpath = "/home/hlsheng/hlsheng_dhale/data/train/sx/"
  lpath = "/home/hlsheng/hlsheng_dhale/data/train/lx/"
  fname ="12.dat"
  ...
  for i in range(4):
      for j in range(5):
          if i%2==0:
            ...
            plt.subplot(4,5,im)
            plt.imshow(np.transpose(lx[:,iq*5,:]),cmap=cm.coolwarm,vmin=colmin, vmax=colmax)
            iq += 1
            im += 1
            plt.colorbar()
  plt.show()
...
def goLayersMap():
  ...
  fpath = "/home/hlsheng/hlsheng_dhale/data_0.1/train/sx/"
  lpath = "/home/hlsheng/hlsheng_dhale/data_0.1/train/lx/"
  fname ="12.dat"
  n1,n2,n3 = 240,192,192
  gx = np.fromfile(fpath+fname,dtype=np.single)# dtype='>f'
  lx = np.fromfile(lpath+fname,dtype=np.single)# dtype='>f'
  ...
  gx = gx/gs
  gx = np.transpose(gx)
  gk = np.zeros((1,n1,n2,n3,1),dtype=np.single)
  gk[0,:n1,:n2,:n3,0] = gx[:,:,:]
  print(model.summary())
  layername = "conv3d_19"
  middle = Model(inputs=model.input,outputs=model.get_layer(layername).output)
  result = middle.predict(gk)[0]
  print(result.shape)

  for i in range(1):
    fx = result[:,:,:,i]
    fx = np.transpose(fx)
    fx.tofile("./map/"+layername+'_'+str(i)+'.dat',format="%4") #  fx.tofile(fpath+"fpd.dat",format="%4")


def goLayersMapEncoder():

  fpath = "/home/hlsheng/co2/deeplearning/net/encoder/"
  fname ="encoder.dat"
  ...
  result = middle.predict(gk)[0]
  print(result.shape)
  np.save('./map/'+"CO2_"+layername+'.npy', result)
  # for i in range(1):
  #   fx = result[:,:,:,i]
  #   fx = np.transpose(fx)
  #   fx.tofile("./map/"+layername+'_'+str(i)+'.dat',format="%4") #  fx.tofile(fpath+"fpd.dat",format="%4")
